...
import net.sf.minuteProject.utils.*;
...
//		}
	...
	public static boolean isToGenerateBasedOnMasterdataAndIsEnum(Template template, GeneratorBean bean) {
		if (bean instanceof Query query) {
			return isMasterDataAndEnumQuery(query);
		}
		return false;
	}

	public static boolean isMasterDataAndEnumQuery(Query query) {
	...
	public static Optional<Query> getQuery(Model model, String queryName) {
		return model.getStatementModel().getQueries().getQueries().stream()
				.filter(q -> q.getName().equals(queryName))
				.findFirst();
	}

	public JavaImportClassVariable getJavaImportClassVariable(Model model, String queryName, Template template, String targetTemplate) {
