...
public class JdbcModelReader
{
     ...
     * @return The columns
     */
    protected Map readColumns(ResultSetMetaData columnData, int i) throws SQLException
    {
    	HashMap values = new HashMap();
    	values.put("COLUMN_NAME", columnData.getColumnName(i));
    	values.put("COLUMN_DEF", columnData.getColumnLabel(i));
    	values.put("DATA_TYPE", columnData.getColumnType(i));
//    	values.put("NUM_PREC_RADIX", columnData.get(i));
    	values.put("COLUMN_SIZE", extractSize(columnData.getColumnDisplaySize(i), columnData.getColumnType(i)));
    	values.put("DECIMAL_DIGITS", columnData.getScale(i));
...
//        return column;
    ...
    private String extractSize(int columnDisplaySize, int columnType) {
    	//To improve
    ...
    private Collection readColumnsByFetch(DatabaseMetaDataWrapper metaData,
			String tableName, Connection connection) {

        ResultSetMetaData columnData = null;
        List columns = new ArrayList();
        try
        {
            columnData = metaData.getColumnsByFetch(tableName, connection);
            for (int i=1; i < columnData.getColumnCount(); i++) {
            	Map values = readColumns (columnData, i);
                columns.add(readColumn(metaData, values));
            }
        }
        catch (Exception e) {
        	//the synonym might not reach at table or view so the sql statement might be
//        	System.out.println("e = "+e);
    ...
    protected Column readColumn(DatabaseMetaDataWrapper metaData, Map values) throws SQLException
    {
        ...
        String size  = (String)values.get("COLUMN_SIZE");
        Integer scl = (Integer)values.get("DECIMAL_DIGITS");
		int    scale = scl.intValue();
